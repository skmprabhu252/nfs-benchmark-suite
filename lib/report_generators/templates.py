...
def get_comparison_only_analysis_html(comparison_insights: List[Dict[str, Any]],
                                     report_style: str) -> str:
    """
    Generate comparison analysis HTML showing ONLY comparison insights in table format.
    
    This template is used when we want to show only ComparisonAnalyzer insights
    without individual test-id health scores or analysis.
    
    Args:
        comparison_insights: Comparison-specific insights from ComparisonAnalyzer
        report_style: Report style for rendering
        
    Returns:
        Comparison analysis HTML with insights in table format
    """
    # Generate comparison insights table
    insights_html = ''
    if comparison_insights:
        # Group insights by test-id and collect workload data
        testid_insights = {}
        workload_data = {}  # {category: {testid: {best_version, improvement}}}
        
        for insight in comparison_insights:
            title = insight.get('title', '')
            description = insight.get('description', '')
            
            # Extract test-id from title (e.g., "tqb: Sequential Read")
            if ':' in title:
                testid = title.split(':')[0].strip()
                category = title.split(':', 1)[1].strip()
                
                if testid not in testid_insights:
                    testid_insights[testid] = []
                testid_insights[testid].append(insight)
                
                # Extract best version and improvement percentage
                lines = description.split('\n')
                best_version_info = lines[0] if lines else ''
                ranking = lines[1].replace('Ranking: ', '') if len(lines) > 1 else 'N/A'
                
                # Parse best version and improvement
                if 'performs best' in best_version_info:
                    parts = best_version_info.split('performs best')
                    best_version = parts[0].strip()
                    improvement = parts[1].strip() if len(parts) > 1 else ''
                else:
                    best_version = 'N/A'
                    improvement = ''
                
                if category not in workload_data:
                    workload_data[category] = {}
                workload_data[category][testid] = {
                    'best_version': best_version,
                    'improvement': improvement,
                    'ranking': ranking
                }
        
        # Create individual test-id tables
        tables_html = ''
        for testid in sorted(testid_insights.keys()):
            insights = testid_insights[testid]
            ...
            for insight in insights:
                ...
                # Extract workload category from title
                if ':' in title:
                    category = title.split(':', 1)[1].strip()
                else:
                    category = title
                
                # Parse description to get best version and ranking
                lines = description.split('\n')
                best_version_info = lines[0] if lines else ''
                ranking = lines[1].replace('Ranking: ', '') if len(lines) > 1 else 'N/A'
            ...
            tables_html += f"""
            <div style="margin-bottom: 30px;">
                <h3 style="color: #667eea; margin-bottom: 15px;">📊 {testid.upper()} Performance by Workload</h3>
                <table>
                    <thead>
                        <tr>
        ...
        # Create cross-test-id comparison table from cross_testid_insights
        # These insights are generated by _analyze_cross_testid_by_category
        cross_testid_insights = [i for i in comparison_insights
                                 if 'nfs_version' in i and 'category' in i]
        
        if cross_testid_insights and len(testid_insights) >= 2:
            # Group by NFS version
            by_version = {}
            for insight in cross_testid_insights:
                nfs_version = insight.get('nfs_version', 'unknown')
                if nfs_version not in by_version:
                    by_version[nfs_version] = []
                by_version[nfs_version].append(insight)
            
            # Create a table for each NFS version
            for nfs_version in sorted(by_version.keys(), key=lambda v: (3, 0) if 'nfsv3' in v.lower() else (4, 0) if 'nfsv4.0' in v.lower() else (4, 1) if 'nfsv4.1' in v.lower() else (4, 2)):
                insights = by_version[nfs_version]
                
                # Get test-id names from the first insight (they're all the same)
                if insights:
                    testid1_name = insights[0].get('testid1_name', 'Test 1')
                    testid2_name = insights[0].get('testid2_name', 'Test 2')
                else:
                    continue
                ...
                for insight in insights:
                    ...
                    testid1_avg = insight.get('testid1_avg', 0)
                    testid2_avg = insight.get('testid2_avg', 0)
                    change_pct = insight.get('change_pct', 0)
                    transport1 = insight.get('transport1', 'tcp')
                    transport2 = insight.get('transport2', 'tcp')
                    
                    better_testid = testid2_name if change_pct > 0 else testid1_name
                    
                    # Format with protocol info
                    testid1_info = f"{testid1_name.upper()}/{transport1.upper()}"
                    testid2_info = f"{testid2_name.upper()}/{transport2.upper()}"
                    
                    # Winner column
                    winner = f"{better_testid.upper()} +{abs(change_pct):.1f}%"
                    
                    comparison_rows += f"""
                    <tr>
                        <td><strong>{category}</strong></td>
                        <td>{testid1_info}: {testid1_avg:.1f}</td>
                        <td>{testid2_info}: {testid2_avg:.1f}</td>
                        <td>{winner}</td>
                    </tr>
                    """
                ...
                tables_html += f"""
                <div style="margin-bottom: 30px;">
                    <h3 style="color: #667eea; margin-bottom: 15px;">⚖️ Cross Test-ID Comparison - {nfs_version.upper()}</h3>
                    <table>
                        <thead>
                            <tr>
                                <th>Workload Category</th>
                                <th>{testid1_name.upper()} Avg Performance</th>
                                <th>{testid2_name.upper()} Avg Performance</th>
                                <th>Winner</th>
                            </tr>
                        </thead>
                        <tbody>
        ...
        insights_html = tables_html
    else:
        insights_html = '<p class="no-analysis">No comparison insights generated.</p>'
    
    return f"""
    <div class="analysis-section">
        <h2>🔍 Comparative Performance Analysis</h2>
        <p style="margin-bottom: 20px; color: #6b7280;">
            This analysis compares performance metrics across NFS versions and test configurations,
            showing which version performs best for each workload category.
        </p>
        {insights_html}
    </div>
    """
